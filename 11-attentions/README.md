...
### 上图整个过程如下：
- 例如 transformer 的维度是[1，12，512],512代表词嵌入的维度 $d_{model}$ ，用reshape拆成8个头后变成[1,12,8,64],64就是 $d_{k}$ ，在每个头上应用Attention机制（知识点4的过程），计算8个头，每个头Tensor的形状都是[1,12,64]，每个头自己进行Attention
- 再用transpose 或者 petmute 把8这个维度提前，变成[1,8,12,64]表示每个句子、8个头、长度为12、嵌入维度 $d_{k}$ = 64，8个12 * 64 的矩阵，每个头之间相互独立了，Q、K、V 都是[1,8,12,64]
- Q : [1,8,12,64]
- K : [1,8,12,64]
- V : [1,8,12,64]
