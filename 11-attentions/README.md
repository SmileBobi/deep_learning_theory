...
- 第一步、计算权重系数：每个Token对其他Token做向量点乘，这里的向量点乘并不是说a1向量直接乘以a2向量，而是每个向量先拆分成Q、K、V三个矩阵，经过线性变化形成q1、k1、v1三个向量，也就是说每个Token对应三个向量，比如图中的a1 Token分别对应q1、k1、v1三个向量，a2 Token、a3 Token、a4 Token也分别对应三个向量（图中没画出来）
